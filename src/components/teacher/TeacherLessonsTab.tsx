...
export default function TeacherLessonsTab({
                                              ...
                                              classes,
                                              subjects,
                                          }: TeacherLessonsTabProps) {
    const [selectedClass, setSelectedClass] = useState<string>("");
    const [selectedSubject, setSelectedSubject] = useState<string>("");

    const filteredLessons = lessons.filter((lesson) => {
        const matchClass = selectedClass ? lesson.classCategory?.id === selectedClass : true;
        const matchSubject = selectedSubject ? lesson.subject?.id === selectedSubject : true;
        ...
        <div className="space-y-6">
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 flex flex-wrap gap-4 items-center">
                ...
                <select
                    ...
                    className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-semibold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500"
                >
            ...
            <div className="space-y-4">
                <h3 className="text-base font-bold text-slate-800">📚 Мої опубліковані уроки</h3>
                {filteredLessons.length === 0 ? (
                    <p className="text-xs text-slate-400">Уроків не знайдено за обраними фільтрами.</p>
                ) : (
                    ...
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-blue-100 text-blue-700">
                      ...
                      {new Date(item.createdAt).toLocaleDateString()}
                                ...
                                <div className="pt-2 border-t border-slate-200 flex justify-between items-center text-xs">
                                    ...
                                    <Link
                                        href={`/teacher/lessons/edit/${item.id}`}
